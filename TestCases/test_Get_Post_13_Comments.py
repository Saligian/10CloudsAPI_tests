...
def test_https_get_post_13_comments():
    url = config.base_url_https + config.post_id_url
...
def test_http_get_post_13_comments():
    url = config.base_url_http + config.post_id_url
    response = requests.get(url)
    json_response = response.json()
    post_id = jsonpath.jsonpath(json_response[0 - 4], 'postId')
    comment_id = jsonpath.jsonpath(json_response[0 - 4], 'id')
